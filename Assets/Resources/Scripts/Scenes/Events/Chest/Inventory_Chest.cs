...
public class Inventory_Chest : MonoBehaviour
{
    ...
    public class Chest_Item
    {
        ...
        public Chest_Item(int item_ID, int maxCount, int chance, int priority)
        {
    ...
    void OnEnable()
    {
        if (initialised)
        {
            ...
            switch (FindObjectOfType<Chest>().slots_Amount)
            {
                case 5:
                    AddReward(new Chest_Item(2, 1, 25, 2), new Chest_Item(5, 1, 25, 2), new Chest_Item(4, 1, 20, 1), new Chest_Item(11, 3, 40, 3));
                    break;
                case 9:
                    AddReward(new Chest_Item(2, 2, 30, 2), new Chest_Item(5, 2, 30, 2), new Chest_Item(4, 2, 50, 1), new Chest_Item(11, 3, 40, 3));
                    break;
                case 12:
                    AddReward(new Chest_Item(2, 2, 40, 2), new Chest_Item(5, 2, 40, 2), new Chest_Item(4, 2, 90, 1), new Chest_Item(11, 3, 40, 3));
                    break;
            }
        }       
    }
    private void Init()
    {
        ...
        inventory_slot = Resources.Load("Prefabs/Slot_Chest", typeof(GameObject)) as GameObject;
        inventory_item = Resources.Load("Prefabs/Item_Chest", typeof(GameObject)) as GameObject;
        ...
        FindObjectOfType<Chest_Tooltip>().inventory = this;
    ...
    private void Initiate_Inventory()
    {
        for (int i = 0; i < 15; i++)
        {
            if (i < 10)
            {
                ...
                inventory_slot_list[i].transform.GetComponent<Data_Slot_Chest>().item_id = i;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F2))
        {
            for (int i = 10; i < 15; i++)
            {
                ...
                {
                    Debug.Log(inventory_slot_list[i].transform.GetChild(0).GetComponent<Data_Item_Chest>().item.Title);
                }
            }
        }
    }
    public void TakeAll()
    {
        if (!SlotsAreFull())
        {
            for (int i = 10; i < 15; i++)
            {
                ...
                if (inventory_slot_list[i].transform.childCount > 0 && inventory_slot_list[i].transform.GetChild(0).GetComponent<Data_Item_Chest>().item.ID != 0)
                {
                    Add_Item(inventory_slot_list[i].transform.GetChild(0).GetComponent<Data_Item_Chest>().item.ID, 0);
                    DeleteItem(i);
                }
    ...
    public void AddReward(params Chest_Item[] chest_Items)
    {
        float randomChance = 0;
        int index = 0;

        chest_Items = chest_Items.OrderBy(w => w.priority).ToArray();

        for (int i = 0; i < chest_Items.Length; i++)
        {
            for (int j = 0; j < chest_Items[i].maxCount; j++)
            {
                randomChance = Random.value * 100;

                if (randomChance < chest_Items[i].chance && index < 5)
                {
                    Add_Item(chest_Items[i].item_ID, 10);
                    index++;
    ...
    private void Add_Item(int id, int index)
    {
        ...
        for (int i = index; i < inventory_item_list.Count; i++)
        {
            if (inventory_item_list[i].ID == 0)
            {
                ...
                new_item.GetComponent<Data_Item_Chest>().item = candidateItem;
                new_item.GetComponent<Data_Item_Chest>().slot = i;
    ...
    public int[] parse_inventory()
    {
        ...
        for (int a = 0; a < 10; a++)
        {
            ...
            {
                inventory[a] = inventory_slot_list[a].transform.GetChild(0).GetComponent<Data_Item_Chest>().item.ID;
