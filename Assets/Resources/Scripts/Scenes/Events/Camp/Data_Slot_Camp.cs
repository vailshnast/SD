...
public class Data_Slot_Camp : MonoBehaviour, IDropHandler, IPointerClickHandler
{

    private Inventory_Camp inventory;
    public int item_id;

    private Button_Proccesor_Camp button_Proccesor;

    void Start()
    {
        inventory = GameObject.Find("Inventory").GetComponent<Inventory_Camp>();
        button_Proccesor = FindObjectOfType<Button_Proccesor_Camp>();
    }

    public void OnDrop(PointerEventData eventData)
    {
        Data_Item_Camp buffered_item = eventData.pointerDrag.GetComponent<Data_Item_Camp>();
        ...
        else
        {
            if (transform.childCount > 0)
            {
                ...
                duplicated_item.GetComponent<Data_Item_Camp>().slot = buffered_item.slot;
                ...
                inventory.inventory_item_list[buffered_item.slot] = duplicated_item.GetComponent<Data_Item_Camp>().item;
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        if (transform.childCount == 0)
        {
            ...
            FindObjectOfType<Camp_Tooltip>().Deactivate(false);
        }
