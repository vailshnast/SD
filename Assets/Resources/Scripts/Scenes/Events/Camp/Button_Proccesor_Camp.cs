...
public class Button_Proccesor_Camp : MonoBehaviour
{

    private Inventory_Camp inventory;
    private Camp_Tooltip tooltip;
    ...
    void Awake()
    {
        ...
        tooltip = gameObject.GetComponent<Camp_Tooltip>();
    }
            ...
            case "Scroll":
                ...
                for (int i = 0; i < inventory.inventory_slot_list.Count; i++)
                {
                    if (inventory.inventory_slot_list[i].transform.childCount > 0)
                    {
                        ...
                        if (inventory.inventory_slot_list[i].transform.GetChild(0).GetComponent<Data_Item_Camp>().item.Group == "Rune" && inventory.inventory_slot_list[i].transform.GetChild(0).GetComponent<Data_Item_Camp>().item.ID == 4)
                        {
